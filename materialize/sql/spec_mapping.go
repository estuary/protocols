...
func generateApplyStatements(
	...
	generator := endpoint.GetGenerator()

	// Like my grandpappy always told me, "never generate a SQL without a comment at the top"
	var comment = generator.CommentRenderer.Render(fmt.Sprintf(
		"Generated by Flow for materializing collection '%s'\nto table: %s",
		spec.Collection.Collection,
		target,
	))

	createSQL, err := generator.CreateTable(
		TableForMaterialization(target, comment, generator.IdentifierRenderer, spec))
	if err != nil {
