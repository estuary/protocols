...
// Log level, you know the drill.
// This is a separate top-level message because it's included in the Config messages of several
// services. It's passed to the Rust side so that log filtering may be perfomed directly in Rust,
// avoiding the need to (de)serialize log messages that would not be published.
type LogLevel int32
...
const (
	...
	// A Log message to be published (Rust -> Go). Note that all LOG messages use the same Code for
	// all service types.
	ExtractAPI_LOG ExtractAPI_Code = 109
)
...
const (
	...
	// A Log message to be published (Rust -> Go). Note that all LOG messages use the same Code for
	// all service types.
	CombineAPI_LOG CombineAPI_Code = 109
)
...
const (
	...
	// A Log message to be published (Rust -> Go). Note that all LOG messages use the same Code for
	// all service types.
	DeriveAPI_LOG DeriveAPI_Code = 109
)
...
type ExtractAPI_Config struct {
	...
	// The current log level, so that log filtering may be done on the Rust side to avoid
	// (de)serialization of log messages that would be ignored.
	LogLevel             LogLevel `protobuf:"varint,5,opt,name=log_level,json=logLevel,proto3,enum=flow.LogLevel" json:"log_level,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
// Stats related to field extraction.
type ExtractAPI_Stats struct {
	Inputs               *DocsAndBytes `protobuf:"bytes,1,opt,name=inputs,proto3" json:"inputs,omitempty"`
...
func (m *ExtractAPI_Stats) Reset()         { *m = ExtractAPI_Stats{} }
func (m *ExtractAPI_Stats) String() string { return proto.CompactTextString(m) }
func (*ExtractAPI_Stats) ProtoMessage()    {}
func (*ExtractAPI_Stats) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{25, 1}
}
func (m *ExtractAPI_Stats) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *ExtractAPI_Stats) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_ExtractAPI_Stats.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *ExtractAPI_Stats) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ExtractAPI_Stats.Merge(m, src)
}
func (m *ExtractAPI_Stats) XXX_Size() int {
	return m.ProtoSize()
}
func (m *ExtractAPI_Stats) XXX_DiscardUnknown() {
	xxx_messageInfo_ExtractAPI_Stats.DiscardUnknown(m)
}

var xxx_messageInfo_ExtractAPI_Stats proto.InternalMessageInfo
...
type CombineAPI_Config struct {
	...
	// The current log level, so that log filtering may be done on the Rust side to avoid
	// (de)serialization of log messages that would be ignored.
	LogLevel             LogLevel `protobuf:"varint,6,opt,name=log_level,json=logLevel,proto3,enum=flow.LogLevel" json:"log_level,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
type DeriveAPI_Config struct {
	...
	// The current log level, so that log filtering may be done on the Rust side to avoid
	// (de)serialization of log messages that would be ignored.
	LogLevel             LogLevel `protobuf:"varint,3,opt,name=log_level,json=logLevel,proto3,enum=flow.LogLevel" json:"log_level,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
type DeriveAPI_Stats struct {
	// Publish lambda invocation stats.
	PublishLambda *DeriveAPI_Stats_InvokeStats `protobuf:"bytes,1,opt,name=publish_lambda,json=publishLambda,proto3" json:"publish_lambda,omitempty"`
	// Update lambda invocation stats.
	UpdateLambda *DeriveAPI_Stats_InvokeStats `protobuf:"bytes,2,opt,name=update_lambda,json=updateLambda,proto3" json:"update_lambda,omitempty"`
	// Number of documents and bytes that were read from source collections.
	Read *DocsAndBytes `protobuf:"bytes,3,opt,name=read,proto3" json:"read,omitempty"`
	// Number of documents and bytes that were output (published) from this derivation.
	Out *DocsAndBytes `protobuf:"bytes,4,opt,name=out,proto3" json:"out,omitempty"`
	...
	RegistersCreated     uint64   `protobuf:"varint,5,opt,name=registers_created,json=registersCreated,proto3" json:"registers_created,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
func (*DeriveAPI_Stats) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{27, 5}
}
...
type DeriveAPI_Stats_InvokeStats struct {
	// Number of times that the lambda was invoked.
...
func (*DeriveAPI_Stats_InvokeStats) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{27, 5, 0}
}
...
// LogEvent messages can be returned from any of the Rust service instances.
// They each represent a log that should be immediately published to an ops log collection.
type LogEvent struct {
	Level LogLevel `protobuf:"varint,1,opt,name=level,proto3,enum=flow.LogLevel" json:"level,omitempty"`
	// Optional structured logging fields, serialized as a JSON object.
	FieldsJson encoding_json.RawMessage `protobuf:"bytes,2,opt,name=fields_json,json=fieldsJson,proto3,casttype=encoding/json.RawMessage" json:"fields_json,omitempty"`
	// Human readable message to be logged.
	Message string `protobuf:"bytes,3,opt,name=message,proto3" json:"message,omitempty"`
	// elapsed_nanos represents the duration from the start of the invocation.
	// Log events must ultimately end up with a timestamp. This raises some questions about uniqueness
	// and monotonicity when integrating logs from both Go and Rust into the same collection. So to
	// avoid any potential issues, we generate all the actual timestamps on the Go side. Go will mark
	// the current time before making each call, and then add this duration to that time to come up
	// with the timestamp for each log event it processes from Rust.
	ElapsedNanos         uint64   `protobuf:"varint,4,opt,name=elapsed_nanos,json=elapsedNanos,proto3" json:"elapsed_nanos,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
func (*IngestRequest) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{31}
}
...
func (*IngestRequest_Collection) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{31, 0}
}
...
func (*IngestResponse) Descriptor() ([]byte, []int) {
	return fileDescriptor_3c1fa740027c1208, []int{32}
}
...
func init() {
	...
	proto.RegisterType((*ExtractAPI_Stats)(nil), "flow.ExtractAPI.Stats")
	...
	proto.RegisterType((*LogEvent)(nil), "flow.LogEvent")
...
var fileDescriptor_3c1fa740027c1208 = []byte{
	...
	,
}
...
func (m *ExtractAPI_Config) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.LogLevel != 0 {
		i = encodeVarintFlow(dAtA, i, uint64(m.LogLevel))
		i--
		dAtA[i] = 0x28
	}
...
func (m *ExtractAPI_Stats) Marshal() (dAtA []byte, err error) {
	size := m.ProtoSize()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *ExtractAPI_Stats) MarshalTo(dAtA []byte) (int, error) {
	size := m.ProtoSize()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *ExtractAPI_Stats) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if m.OutputBytes != 0 {
		i = encodeVarintFlow(dAtA, i, uint64(m.OutputBytes))
		i--
		dAtA[i] = 0x10
	}
	if m.Inputs != nil {
...
func (m *DeriveAPI_Stats) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.RegistersCreated != 0 {
		i = encodeVarintFlow(dAtA, i, uint64(m.RegistersCreated))
		i--
		dAtA[i] = 0x28
	}
	if m.Out != nil {
...
func (m *DeriveAPI_Stats_InvokeStats) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.MaxSeconds != 0 {
		i -= 8
		encoding_binary.LittleEndian.PutUint64(dAtA[i:], uint64(math.Float64bits(float64(m.MaxSeconds))))
		i--
		dAtA[i] = 0x21
	}
	if m.MinSeconds != 0 {
		i -= 8
		encoding_binary.LittleEndian.PutUint64(dAtA[i:], uint64(math.Float64bits(float64(m.MinSeconds))))
		i--
		dAtA[i] = 0x19
	}
	if m.TotalSeconds != 0 {
	...
	if m.Count != 0 {
		i = encodeVarintFlow(dAtA, i, uint64(m.Count))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func (m *LogEvent) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.ElapsedNanos != 0 {
		i = encodeVarintFlow(dAtA, i, uint64(m.ElapsedNanos))
		i--
		dAtA[i] = 0x20
	}
	if len(m.Message) > 0 {
		...
		i = encodeVarintFlow(dAtA, i, uint64(len(m.Message)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.FieldsJson) > 0 {
		i -= len(m.FieldsJson)
		copy(dAtA[i:], m.FieldsJson)
		i = encodeVarintFlow(dAtA, i, uint64(len(m.FieldsJson)))
		i--
		dAtA[i] = 0x12
	}
	if m.Level != 0 {
...
func (m *ExtractAPI_Stats) ProtoSize() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.Inputs != nil {
		l = m.Inputs.ProtoSize()
		n += 1 + l + sovFlow(uint64(l))
	}
	if m.OutputBytes != 0 {
		n += 1 + sovFlow(uint64(m.OutputBytes))
	}
	if m.XXX_unrecognized != nil {
		n += len(m.XXX_unrecognized)
	}
...
func (m *DeriveAPI_Stats) ProtoSize() (n int) {
	...
	if m.RegistersCreated != 0 {
		n += 1 + sovFlow(uint64(m.RegistersCreated))
	}
	if m.XXX_unrecognized != nil {
		n += len(m.XXX_unrecognized)
	}
...
func (m *LogEvent) ProtoSize() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if m.ElapsedNanos != 0 {
		n += 1 + sovFlow(uint64(m.ElapsedNanos))
	}
	if m.XXX_unrecognized != nil {
		n += len(m.XXX_unrecognized)
	}
...
func (m *ExtractAPI_Stats) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
...
func (m *DeriveAPI_Stats) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if m.PublishLambda == nil {
				m.PublishLambda = &DeriveAPI_Stats_InvokeStats{}
			}
			if err := m.PublishLambda.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
		...
		case 2:
			...
			if m.UpdateLambda == nil {
				m.UpdateLambda = &DeriveAPI_Stats_InvokeStats{}
			}
			if err := m.UpdateLambda.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
		...
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field RegistersCreated", wireType)
			}
			m.RegistersCreated = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowFlow
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RegistersCreated |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *LogEvent) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field FieldsJson", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.FieldsJson = encoding_json.RawMessage(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
		...
		case 4:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ElapsedNanos", wireType)
			}
			m.ElapsedNanos = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ElapsedNanos |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
